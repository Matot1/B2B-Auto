...
Синяя кнопка Code → скопировать URL (Clone with HTTPS).

Terminal и вставь свой URL вместо ...:

`cd ~/Desktop` `git clone ...`

Появится папка на Рабочем столе — это и есть папка проекта. Перенести файлы с заменой в эту папку.

В Terminal:

`cd ~/Desktop/b2b-autotest-js` `git add -A` `git commit -m "Update files"` `git push`
